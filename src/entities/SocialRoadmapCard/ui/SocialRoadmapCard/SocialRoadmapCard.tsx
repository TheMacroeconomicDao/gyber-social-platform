...
import {ArrowFinal, ArrowLeft, ArrowRight} from "@/entities/SocialRoadmapCard/ui/SocialRoadmapCard/Arrows";
    ...
    const items = stages.map(stage => (
        {
            ...
            status:
                stage.status === 'done'
                ? IconStatus.DONE
                : (stage.status === 'progress' ? IconStatus.PROGRESS : IconStatus.DEFAULT)
        }
    ))
    return (
        <div className={classNames(cls.SocialRoadmapCard, {}, [className])}>
            <SquareCard>
                {items.map(item => <StageItem key={item.title} {...item}/>)}
                <p>{description}</p>
            </SquareCard>
